...
`Header.html` provides the semantic template, `Header.css` contains scoped desktop/tablet/mobile styles, and `Header Integration.js` populates the component from the existing WordPress logo and primary menu. Run `npm run build:header` to generate `Header Embed.html`, `Header Bootstrap.html`, and update only the marked component block at the end of `Footer.html`. Run `npm run check` and `npm test` before deployment.
...
Desktop navigation fits on one row; below 980px it becomes a disclosure menu. Search matches the chapter's public pages, events, and resources without exposing legacy WordPress posts. Keyboard controls support Tab, Enter, Arrow Down for Members, and Escape with focus return. Current-page states use WordPress metadata and the actual page URL.
...
To deploy, push the component assets to GitHub Pages, wait for deployment, then append `Header Bootstrap.html` to **Settings → Advanced Settings → Additional code → Header Tag (high priority)** and append `Header Embed.html` to **Footer code** (inside the `ASME HEADER COMPONENT START/END` comments). Preserve all existing shared code and the original stylesheet link. The bootstrap prevents the original WordPress header flashing during fast navigation and restores it after 3.5 seconds if the component cannot load. No page editor changes are needed. To revert the header alone, remove both marked blocks; the native WordPress header remains available as its fallback.
...
The album-list collector returns photo IDs, dimensions, dates, and image URLs but not descriptions. The sync therefore reads each photo's public detail page and extracts the description displayed in Google Photos' info panel. A nonempty public description takes precedence over `data/google-photos-titles.json`, which maps stable photo IDs to fallback titles for photos without descriptions. If a detail page fails or Google changes its undocumented page format, the last published title is retained where possible. The generated `alt` field feeds the gallery hover label and lightbox. The footer's image-orientation classifier remains in place; the browser packs adjacent photos into variable-height, 12-column justified rows according to their dimensions, filling each row without changing date order. Thumbnails crop to their cells; the lightbox retains the full image.
...
`Gallery Integration.js` merges `data/google-photos-feed.json` into the existing WordPress archive grid. Imported photos use locally hosted WebP thumbnail and large variants, work with the archive filters/lightbox, and never alter the 13 WordPress attachments or the Instagram section.
...
Run `npm run sync:google-photos -- --dry-run` to test anonymous collection. The collector parses only the public page's structured bootstrap data—without evaluating Google code—and checks its resolved album ID, title, share key, and advertised total media count. It then uses the pinned maintained collector to follow the documented-by-observation `snAcKc` continuation requests until their server-issued next-page token ends. Publication is allowed only when unique returned stable media IDs exactly equal the bootstrap total. A login page, malformed bootstrap, repeated/broken continuation, parse failure, or count mismatch fails the run and preserves the prior manifest/assets; zero is treated as empty only when the trusted bootstrap total is explicitly zero. Videos are skipped after complete enumeration and reported in the workflow log.

The protocol was locally verified anonymously on September 8, 2026: the intended album returned four stable still-photo IDs with bootstrap total `4`; an unrelated public validation album returned 722 stable IDs while its initial bootstrap page contained 300. This validates real multi-page traversal without mutating the ASME source album. It remains an undocumented Google Photos web protocol, not a supported Library API; officers must treat a collection failure as a safe no-op and investigate before rerunning.

After a successful complete read, removals apply only to this integration's generated assets after a second matching read for an empty or unusually large deletion. Removing an item from Google Photos removes its hosted gallery copy after the next successful sync; it does not erase WordPress/Instagram copies, Git history, third-party caches, or the Google original. Roll back by reverting the generated manifest/assets commit and disabling the workflow.

Installation order: (1) merge/push this repository change; (2) replace the GeneratePress footer element with the complete current `Footer.html` so its delegated lightbox can see asynchronously added photos; (3) keep the existing Gallery page script tag exactly as it is—`Gallery Page.html` already loads `Gallery Integration.js`; (4) use **Run workflow** for the Google Photos workflow; and (5) after a successful run, open `https://asme-osu.github.io/ASME-OSU-Website/data/google-photos-feed.json` and `https://org.osu.edu/asme/pictures/` in a signed-out browser to verify the manifest and gallery. Do not paste a second gallery widget, iframe, or shortcode into WordPress.
...
- Rank trend arrows compare the current export with the prior distinct, same-semester snapshot in `data/leaderboard-rank-snapshots.json`. The scheduled **Publish public leaderboard rank snapshot** workflow reads only the sanitized Website Export, publishes only public display-name/rank pairs, and never advances a baseline for a request-time change alone. On the first published snapshot, a period rollover, a duplicate display name, or a stale/mismatched snapshot, movement is deliberately unavailable rather than inferred from browser history. Do not put emails, OSU name.numbers, or private workbook data in this file.
