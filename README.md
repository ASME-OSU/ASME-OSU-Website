...
The public shared-album collector currently returns photo IDs, dimensions, dates, and image URLs, but **not Google Photos descriptions**. Adding a description in Google Photos alone therefore cannot yet update the website title. To label a photo now, create `data/google-photos-titles.json` as an object mapping the stable `id` from `data/google-photos-feed.json` to a short public title, for example `{"AF1Qip...":"ASME fall kickoff"}`. Commit it and run the Google Photos workflow; the generated `alt` field feeds both the gallery hover label and the lightbox. If the collector exposes `description` in a future release, that source description takes precedence automatically. Imported thumbnails are kept to consistent grid cells while the lightbox retains the full image.
